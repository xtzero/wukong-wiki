...
const processOriginUrl = url => {
    return '图片和游戏内资料整理自游民星空,考据整理自B站黑神话：悟空WIKI。'
}
onMounted(async () => {
    menuButtonRect.value = wx.getMenuButtonBoundingClientRect()
    data.value = (await wukongData.loadData('rubys')).map(v => ({
        ...v,
        ...
        <view
            ...
            <view
                ...
                <view class="ruby-right">
                    <view class="ruby-name">{{ v.name }}</view>
                    <view class="ruby-text">{{ v.level }} - {{ v.text }}</view>
                </view>
            </view>
        </view>
